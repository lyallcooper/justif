...
function parseArgs(args) {
  ...
  if (options.dryRun && options["github-output"]) {
    throw new Error("--dry-run and --github-output cannot be combined");
  }
  return options;
}
...
function main() {
  ...
  // CI must exercise the same real tarball path used for publishing.
  // `--dry-run` means discard that artifact after validation, not ask npm to
  // simulate packing: npm's simulated file list has omitted nested dist files.
  const temporaryPackDestination = options.dryRun
    ? mkdtempSync(join(tmpdir(), "justif-package-"))
    : null;
  const packDestination =
    options["pack-destination"] ?? temporaryPackDestination;
  if (!packDestination) {
  ...
  try {
    ...
    const tarball = resolve(packDestination, validated.filename);
    ...
    if (temporaryPackDestination) {
      rmSync(temporaryPackDestination, { recursive: true, force: true });
    }
  }
}
